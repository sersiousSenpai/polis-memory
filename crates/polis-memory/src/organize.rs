...
pub async fn organize_once_with(polis: &Polis<'_>, oracle: &dyn SimilarityOracle) -> Result<OrganizeOutcome, String> {
    ...
    if !filing.consolidation_due {
        ...
        return Ok(OrganizeOutcome {
            ...
            ran: true,
            run_id: Some(run_id),
            // A plain (non-consolidation) run applies no structural op: the
            // adjudicated queue (B3) is processed on the consolidation runs
            // the filing tier schedules, which is where structural ops belong.
            refused: 0,
            applied: 0,
            deferred: 0,
            expired: 0,
