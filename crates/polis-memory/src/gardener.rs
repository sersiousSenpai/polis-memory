...
mod step_tests {
    ...
    async fn the_canary_reverts_a_run_that_made_the_memory_less_reachable() {
        ...
        let mut st = GardenerState::default();
        // C1 put the deterministic filing tier in front of the classifier: a
        // plain run files and returns without consolidating, so the collapse
        // this test scripts is only reachable on a consolidation run. Make
        // this one due the way the gardener itself would (health pressure,
        // or the fifth organize).
        store.set_meta(crate::filing::HEALTH_PRESSURE_KEY, "1").unwrap();
        store.set_meta(crate::filing::ORGANIZE_COUNT_KEY, &(crate::filing::CONSOLIDATE_EVERY - 1).to_string()).unwrap();
