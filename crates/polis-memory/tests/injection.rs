...
async fn an_obedient_model_changes_nothing_it_was_not_shown() {
    ...
    // C1's filing tier runs first and a plain run never reaches the
    // classifier; the adversary is only heard on a consolidation run, so
    // make this one due the way the gardener itself would.
    store.set_meta(polis_memory::filing::HEALTH_PRESSURE_KEY, "1").unwrap();
    store.set_meta(polis_memory::filing::ORGANIZE_COUNT_KEY, &(polis_memory::filing::CONSOLIDATE_EVERY - 1).to_string()).unwrap();
