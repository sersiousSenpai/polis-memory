...
//! its reads, writes and nested MCP service with a bearer token. What the `polis serve` command runs; a
...
//! all routes require a credential when a token is configured. With no token,
//! only open and hook-contract routes pass; writes and MCP remain disabled.
//! A route the router serves that is not in the table fails
...
/// The token required across the surface. `None` permits only open/hook routes
/// and is allowed on loopback only.
...
pub fn authorize(path: &str, method: &str, bearer: Option<&str>, token: Option<&str>) -> Result<(), Denial> {
    ...
    let scope = if path == "/mcp" || path.starts_with("/mcp/") {
        "memory.mcp"
    } else {
        let spec = route_spec(path, method).ok_or(Denial::UnknownRoute)?;
        match spec.class {
            RouteClass::Open | RouteClass::HookContract if token.is_none() => return Ok(()),
            RouteClass::Open | RouteClass::HookContract => "memory.read",
...
pub async fn require_token(State(auth): State<Arc<StandaloneAuth>>, req: Request, next: Next) -> Response {
    // Nested services (including Streamable HTTP MCP) may have no MatchedPath.
    let path = req.extensions().get::<MatchedPath>().map(|p| p.as_str()).unwrap_or_else(|| req.uri().path()).to_string();
...
pub fn app(state: PolisState, auth: StandaloneAuth) -> Router {
    guard(crate::router::<PolisState>().with_state(state), auth)
}

/// Apply once, after the HTTP and MCP services have both been assembled.
pub fn guard(router: Router, auth: StandaloneAuth) -> Router {
    router.layer(axum::middleware::from_fn_with_state(Arc::new(auth), require_token))
}
...
pub fn check_bind(addr: &SocketAddr, auth: &StandaloneAuth) -> Result<(), String> {
    if !addr.ip().is_loopback() && auth.token.as_deref().is_none_or(|t| t.trim().is_empty()) {
        return Err(format!(
...
mod tests {
    ...
    fn open_and_hook_routes_pass_writes_need_the_token_unknown_fails_closed() {
        let tok = Some("secret");
        assert!(matches!(authorize("/v1/memory/tree", "GET", None, tok), Err(Denial::MissingToken { .. })));
        assert!(matches!(authorize("/v1/prompts/ingest", "POST", None, tok), Err(Denial::MissingToken { .. })));
